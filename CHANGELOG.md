...
### Changed
- **Default behavior for cross-drive swaps is now REL** (preserve relative path)
  - Same-drive swaps still default to SAME (swap contents in place)
- Always display file destinations when paths are changing (not just verbose mode)

### Added
- Unit tests for path preservation helper functions
- One-off test script for verifying path preservation behavior
